...
BDCRequest.prototype.post = function(url, params, cb) {
  var postParams = merge(params, this.Credentials);

  console.log("IN POST FOR " + url);
...
BDCRequest.prototype.create = function(object_url, params, cb) {
  var self = this;
  var command = "Crud/Create/" + object_url;
  params = {obj:JSON.stringify(params)};
  self.request(command,params,cb);
};

BDCRequest.prototype.update = function(object_url, params, cb) {
  var self = this;
  var command = "Crud/Update/" + object_url;
  params = {obj:JSON.stringify(params)};
  self.request(command,params,cb);
};

BDCRequest.prototype.read = function(command, id, cb) {
  var self = this;
  params = {id:id};
  self.request(command,params,cb);
};
